...
def plot_umap_clusters(input_csv: str, labels_path: str, output_path: str) -> None:
    ...
    # The JSON keys are strings ('-1', '0', etc.),
    # so we must cast the ID column to string first.
    # We also handle the -1 cluster (noise) by giving it a specific name.
    df["cluster_name"] = (
        df["denial_cluster_id"]
        .astype(str)
        .map(cluster_label_map)
        .fillna("Noise - Unclustered")
    )
    ...
    sns.scatterplot(
        ...
        ),  # Ensures consistent legend order
