...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Assign clusters to new claims using a pre-trained model.")
    ...
    if df.empty:
        ...
        pd.DataFrame(columns=list(df.columns) + ['denial_cluster_id', 'umap_x', 'umap_y']).to_csv(args.output, index=False)
        exit(0)

    # --- FIX: Unpack both the labels and the coordinates ---
    predicted_labels, umap_coords_df = predict_clusters(
        raw_data=df,
        reducer_path=args.reducer_model,
        clusterer_path=args.cluster_model
    )
