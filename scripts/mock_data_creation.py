...
def create_mock_training_data_v5() -> None:
    """
    ...
    notes_map = {
        "auth": [
            "Auth team contacted; no auth found.",
            "Retro-auth requested.",
            "Escalated to payer auth desk.",
        ],
        "necessity": [
            "Appeal letter drafted.",
            "Peer-to-peer review scheduled.",
            "Clinical notes submitted for review.",
        ],
        "misc": [
            "Billing team correcting COB info.",
            "Duplicate claim voided.",
            "Corrected claim pending resubmission.",
        ],
        "eligibility": [
            "Eligibility verified with payer.",
            "Patient asked to provide updated ID card.",
            "Coverage terminated notice received.",
        ],
        "coding": [
            "Coding team reviewing ICD/CPT mapping.",
            "Modifier correction submitted.",
            "Re-billed with corrected diagnosis code.",
        ],
    }
    ...
    for i in range(1, num_records + 1):
        ...
        patient_dob = datetime.now() - timedelta(
            days=random.randint(365 * 20, 365 * 70)
        )
        denial_date = (
            (service_date + timedelta(days=random.randint(2, 40))).strftime("%Y-%m-%d")
            if is_denied
            else None
        )
        ...
        record = {
            ...
            "provider_type": random.choice(
                ["Cardiology", "Oncology", "Primary Care", "Orthopedics", "Radiology"]
            ),
            ...
            "billing_provider_specialty": random.choice(
                ["Cardiology", "Oncology", "Primary Care", "Orthopedics", "Radiology"]
            ),
            "facility_code": random.choice([11, 21, 22, 23]),
            "plan_type": random.choice(["private", "medicare", "medicaid"]),
            "diagnosis_code_primary": random.choice(
                ["Z00.00", "E11.9", "M54.5", "I10", "J44.9"]
            ),
            "procedure_code": random.choice(
                [99213, 99214, 85025, 93000, 99203, 99204, 36415]
            ),
        }
        data.append(record)
